	...
	SubShader {
		...
		Pass {
			...
			float LinearDepthToUnityDepth(float linearDepth)
			{
				...
				float depth01 = saturate((linearDepth - _ProjectionParams.y) / (_ProjectionParams.z - _ProjectionParams.y));
			...
			{
				// // On sphere billboard fragment shader
				// float2 centerOffset = (i.uv - 0.5) * 2; // tangent-space x/y

				// float sqrDst = dot(centerOffset, centerOffset);
				// if (sqrDst > 1) discard;

				// float z = sqrt(saturate(1.0 - dot(centerOffset, centerOffset)));
				// float3 localNormal = normalize(float3(centerOffset, z));
				...
				// 	camRight * localNormal.x +
				// 	camUp * localNormal.y +
				// 	camForward * localNormal.z
				// );

				// return float4(worldNormal * 0.5 + 0.5, 1.0); // visualize
				...
				float visual = saturate((linearDepth - _ProjectionParams.y) / (_ProjectionParams.z - _ProjectionParams.y));

				return visual;  // 0 is near, 1 is far
			}
